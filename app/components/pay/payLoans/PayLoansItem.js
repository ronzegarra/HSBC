...
export default class PayLoansItem extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  ...
  viewMore(item) {
    console.warn(item.Loans.length);

    if (item.Type === 'CC')
      this.props.navigation.navigate('PagarPrestamo_Segunda', {
        data: item.Loans,
        lastItem: item.Loans[item.Loans.length - 1]
      });
  }
